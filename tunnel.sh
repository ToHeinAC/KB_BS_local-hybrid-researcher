#!/bin/bash
# Expose Hybrid Researcher (port 8511) via a Cloudflare Quick Tunnel (no account needed).
# Pattern from local_summarizer/tunnel.sh, adapted to this app's port + paths.
#
...
PORT=8511
LOG=/tmp/hybrid-researcher-tunnel.log
APP_LOG=/tmp/hybrid-researcher-app.log
URL_FILE=/tmp/hybrid-researcher-app-url.txt
CONFIG=~/.cloudflared/config.yml
CRED_FILE=~/.cloudflared/5bc36850-dc39-48be-81ab-fd15f8071bd0.json

APP_PATTERN="streamlit run src/ui/app.py --server.port $PORT"
TUNNEL_PATTERN="cloudflared tunnel --url http://localhost:$PORT"

if [ "$1" = "stop" ]; then
  echo "Stopping Hybrid Researcher and its tunnel (port $PORT)..."
...
echo "Starting Cloudflare Quick Tunnel for Hybrid Researcher (port $PORT)"
echo "===================================================================="
...
if ! lsof -ti:"$PORT" >/dev/null 2>&1; then
  echo "App not running — starting Hybrid Researcher on port $PORT..."
  setsid nohup uv run streamlit run src/ui/app.py --server.port "$PORT" --server.headless true \
...
if [ -n "$URL" ]; then
  ...
  echo "TUNNEL READY"
  echo ""
  echo "  Public URL: $URL/brain/"
  echo "  (baseUrlPath is 'brain' — the port root 404s, see .streamlit/config.toml)"
  echo ""
